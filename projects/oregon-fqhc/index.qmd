...
## Project Status
**Phase 4: Automated QA & Testing**

- [View Technical Architecture &rarr;](about.qmd)

Data sourced from:
- [HRSA Data Warehouse](https://data.hrsa.gov/) (Site Locations)
- 2024 UDS Table 4 (Patient Demographics)

Data validated via `pytest` before every deployment
```{python}
#| label: setup
#| warning: false
#| echo: false
...
### Analysis: Scale vs. Safety Net

Is there a relationship between the size of a clinic and its payer mix?

The data reveals a **weak positive correlation (0.27)** between clinic size and Medicaid volume. Larger clinics tend to serve a slightly higher percentage of Medicaid patients, though there is significant variance among smaller sites.
...
#| fig-cap: "Correlation: Patient Volume vs. Medicaid %"
#| warning: false

# Filter out tiny sites for cleaner visualization
df_trend = df[df['total_patients'] > 100]
...
fig_corr = px.scatter(
    ...
    y="pct_medicaid",
    hover_name="site_name",
    hover_data={"total_patients": False, "pct_medicaid": False, "Patients (fmt)": True, "Medicaid %": True},
    trendline="ols", # <--- Ordinary Least Squares Regression Line
    labels={"total_patients": "Total Patients", "pct_medicaid": "Medicaid %"},
    title="Do Larger Clinics Serve More Medicaid Patients?",
    height=500
)
